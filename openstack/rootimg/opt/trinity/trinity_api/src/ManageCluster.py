...
class Modify (object):
    ...
    def modify_cluster(self,cluster,version=1):
      ...
      all_nodes_info=api.all_nodes_info
      ...
      ##---------------------------------------------------------------------
     #nova = novaclient.Client('2', req.keystone_admin_user, req.keystone_admin_pass,
      ...
      xcat_networks=requests.get(req.xcat_host+path,verify=False,params=req.query,headers=req.headers).json()["networks"]
      network_map={}
      ...
      if not cluster_exists :
        ...
        subprocess.call('makedns login_cluster',shell=True)
        subprocess.call('makehosts login_cluster', shell=True)
        subprocess.call('makehosts vc_cluster',shell=True)
      ...
      if cont_subs_list:
        subprocess.call('makedns -d cont_subs_string',shell=True)
      ...
      if cont_adds_list:
        subprocess.call('makehosts cont_adds_string',shell=True)
        subprocess.call('makedns cont_adds_string',shell=True)
      ...
      if not nova.servers.list(search_opts={'all_tenants': 1, 'name': 'login-' + cluster}):
        ...
        for role in r.json()["roles"]:
          if role["name"] ==req.keystone_admin_user:
            role_id=role["id"]
            break
        ...
        for user in r.json()["users"]:
          if user["username"] ==req.keystone_admin_user:
            user_id=user["id"]
            break
        ...
        payload = { 
                   "auth": {
                     "tenantName": cluster,
                     "passwordCredentials": {
                       "username": req.keystone_admin_user,
                       "password": req.keystone_admin_pass
                     }
                   }
                 }
        ...
        path=req.nova_host+'/'+req.tenant_id+'/servers'
        headers={"X-Auth-Project-Id":cluster, "X-Auth-Token":tenant_token}
        headers.update(req.headers)
        payload={
          "server":{
